...
class OrderSummaryController extends Controller {
    ...
    public function actionReport() {
        ...
        if (isset($_GET['SaveExcel'])) {
            $this->saveToExcel($orderReport, $orderReport->dataProvider, array('startDate' => $startDate, 'endDate' => $endDate));
        }
    ...
    protected function saveToExcel($orderReport, $dataProvider, array $options = array()) {
        ...
        foreach ($dataProvider->data as $header) {
            ...
            $worksheet->setCellValue("A{$counter}", CHtml::encode($header->number));
            $worksheet->setCellValue("B{$counter}", CHtml::encode($header->reference_number));
            $worksheet->setCellValue("C{$counter}", CHtml::encode($header->date));
            ...
            $worksheet->setCellValue("G{$counter}", CHtml::encode(CHtml::value($header, 'grandTotal')));
            $worksheet->setCellValue("H{$counter}", CHtml::encode(CHtml::value($header, 'admin.username')));
        ...
        header('Content-Disposition: attachment;filename="Laporan Order Penjualan.xlsx"');
