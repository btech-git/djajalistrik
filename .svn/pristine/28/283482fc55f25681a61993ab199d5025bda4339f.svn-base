<?php if ($error === true && count($saleReceipt->details) === 0): ?>
...
<table style="border: 1px solid">
    ...
    <?php foreach ($saleReceipt->details as $i => $detail): ?>
        ...
        <?php $detailInvoice = $detail->invoiceHeader(array('scopes' => 'resetScope', 'with' => 'orderHeader:resetScope')); ?>

        <tr style="background-color: #FFF8DC">
            <td>
                <?php echo CHtml::activeHiddenField($detail, "[$i]invoice_header_id"); ?>
                <?php echo CHtml::encode(CHtml::value($detailInvoice, 'number')); ?>
            </td>
            <td><?php echo CHtml::encode(CHtml::value($detailInvoice, 'tax_number')); ?></td>
            <td><?php echo CHtml::encode(CHtml::value($detailInvoice, 'orderHeader.reference_number')); ?></td>
            <td><?php echo CHtml::encode(Yii::app()->dateFormatter->format('d MMMM yyyy', strtotime(CHtml::value($detailInvoice, 'date')))); ?></td>
    ...
    <tr style="background-color: #F5DEB3">
        ...
        <td style="text-align: right; font-weight: bold">
            <?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0.00', $saleReceipt->totalDetail)); ?>
        </td>
        <td></td>
    </tr>
</table>