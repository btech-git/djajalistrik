...
class Transfer extends CComponent {
    ...
    public function updateProducts() {
        foreach ($this->details as $detail)
            $detail->product->getStock($this->header->warehouse_id_from);
    }
...
//				'warehouse_id' => $this->header->warehouse_id_to,
...
//				$inventory->transaction_subject = $this->header->warehouse_id_to;
...
//			$inventory->warehouse_id = $this->header->warehouse_id_to;
//
...
//				'transaction_number' => $this->header->number,
//				'product_id' => $detail->product_id,
//				'warehouse_id' => $this->header->warehouse_id_from,
//				'branch_id' => $this->header->branch_id,
//				'transaction_type' => 6,
//				));
//			if ($inventoryFound === null)
...
//				$inventory->transaction_number = $this->header->number;
//				$inventory->transaction_type = 6;
//				$inventory->transaction_subject = $this->header->warehouse_id_from;
//				$inventory->product_id = $detail->product_id;
//				$inventory->unit_id = $detail->unit_id;
//				$inventory->admin_id = $this->header->admin_id;
//				$inventory->branch_id = $this->header->branch_id;
...
//			$inventory->warehouse_id = $this->header->warehouse_id_from;
//
//			$valid = $inventory->save(false) && $valid;
//		}
//
    ...
    public function flush() {
        ...
        foreach ($this->details as $detail) {
            ...
            if ((int) $detail->is_inactive === ActiveRecord::ACTIVE) {
                ...
                $inventory->transaction_number = $this->header->number;
                $inventory->transaction_type = 6;
                $inventory->transaction_subject = $this->header->warehouseIdFrom->name;
                $inventory->product_id = $detail->product_id;
                $inventory->unit_id = $detail->unit_id;
                $inventory->admin_id = $this->header->admin_id;
                $inventory->branch_id = $this->header->branch_id;
