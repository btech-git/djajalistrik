...
Yii::app()->clientScript->registerScript('admin', "
    $('form').submit(function(){
        $.fn.yiiGridView.update('purchase-return-grid', {
            data: $(this).serialize()
        });
        return false;
    });
...
<h1>Kelola Retur Pembelian Barang</h1>
<div id="detail_div">
    <p>
    ...
    <div>
        ...
        <?php echo CHtml::button('Reset', array('onclick' => '$("#PurchaseReturnHeaderDate").val(""); $("form").submit();')); ?>
    </div>
    ...
    <?php $this->widget('zii.widgets.grid.CGridView', array(
        ...
        'columns' => array(
            ...
            array(
                'header' => 'Supplier',
                'filter' => CHtml::dropDownList('SupplierId', $supplierId, CHtml::listData(Supplier::model()->findAll(), 'id', 'name'), array('empty' => '')),
                'value' => '$data->getSupplierCompany()',
            ),
            array(
                'name' => 'receive_header_id',
//                'filter' => CHtml::listData(ReceiveHeader::model()->findAll(), 'id', 'number'),
                'value' => '$data->getReceiveHeaderNumber()',
            ),
            array(
                'class' => 'CButtonColumn',
