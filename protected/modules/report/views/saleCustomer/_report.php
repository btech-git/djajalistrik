...
<table class="report">
    ...
    <?php foreach ($saleCustomerReport->dataProvider->data as $header): ?>
        <tr class="items1">
        ...
        <tr class="items2">
            <td colspan="2">
                <table>
                    <?php $totalSale = 0; ?>
                    <?php foreach ($header->orderHeaders as $detail): ?>
                        <?php if ($detail->date >= $startDate && $detail->date <= $endDate) : ?>
                            <tr>
                                <td class="width2-1"><?php echo CHtml::encode(CHtml::value($detail, 'number')); ?></td>
                                <td class="width2-2"><?php echo CHtml::encode(Yii::app()->dateFormatter->format('d MMMM yyyy', strtotime($detail->date))); ?></td>
                                <td class="width2-3" style="text-align: right"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', $grandTotal = CHtml::value($detail, 'grandTotal'))); ?></td>
                            </tr>
                    ...
                    <tr>
                        ...
                        <td class="width2-3" style="border-top: 1px solid; font-weight: bold; text-align: right"><?php echo CHtml::encode(Yii::app()->numberFormatter->format('#,##0', ceil($totalSale))); ?></td>
                    </tr>
                </table>
            </td>
        </tr>
